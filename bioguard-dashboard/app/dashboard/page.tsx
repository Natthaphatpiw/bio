          ...
          <div className="flex items-center gap-4">
            <Link href="/" className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Verification Dashboard</p>
              </div>
            </Link>
          </div>
          <Link
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="px-6 py-4 text-left text-sm text-slate-500 font-medium">Session ID</th>
                <th className="px-6 py-4 text-left text-sm text-slate-500 font-medium">Time</th>
                <th className="px-6 py-4 text-left text-sm text-slate-500 font-medium">Status</th>
                <th className="px-6 py-4 text-left text-sm text-slate-500 font-medium">Environment</th>
                <th className="px-6 py-4 text-left text-sm text-slate-500 font-medium">Light-Sync</th>
                <th className="px-6 py-4 text-left text-sm text-slate-500 font-medium">Face Liveness</th>
              </tr>
            </thead>
            <tbody>
              {filteredSessions.map((session) => (
                <tr key={session.id} className="border-b border-slate-200 hover:bg-slate-50">
                  <td className="px-6 py-4">
                    <span className="font-mono text-sm text-slate-700">{session.id.slice(0, 8)}...</span>
                  </td>
                  <td className="px-6 py-4">
                    <div>
                      <p className="text-sm text-slate-700">{formatTime(session.created_at)}</p>
                  ...
                  <td className="px-6 py-4">
                    {session.result?.environment ? (
                      <span className={session.result.environment.isSafe ? 'text-emerald-600' : 'text-rose-600'}>
                  ...
                  <td className="px-6 py-4">
                    {session.result?.lightSync ? (
                      <div className="flex items-center gap-2">
                        <span className={session.result.lightSync.pass ? 'text-emerald-600' : 'text-rose-600'}>
                  ...
                  <td className="px-6 py-4">
                    {session.result?.faceLiveness ? (
                      <div className="flex items-center gap-2">
                        <span className={session.result.faceLiveness.isReal ? 'text-emerald-600' : 'text-rose-600'}>
